...
This repository is also packaged as a combined plugin. The existing portable
skill remains the governing intelligence layer; the MCP adapter exposes its
authoritative intent checkpoint, lane session, evidence, PolicyGuard, feedback,
and resume controls to ChatGPT/Codex without duplicating the engine.

See [docs/PLUGIN.md](docs/PLUGIN.md) for the local install, protocol validation,
hosted transport, tool truth contract, and the capabilities that are not yet
claimed as shipped.
